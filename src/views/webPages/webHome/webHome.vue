  ...
  <div class='webHome'>
    <div id='webHome'>
      <div class='main-content col-9'>
        <div v-for='(i,index) in state.list.rows' :class="index < 10?'animate__fadeInUp':''" class='card main-item animated animate__animated cursor' @click='state.show(i)'>
          <p class='title cursor'>{{ i.title }}</p>
          <p class='author cursor'>{{ i.author }}</p>
          <p class='introduction omit4 cursor'>{{ i.introduction }}{{ i.introduction }}{{ i.introduction }}{{
              i.introduction
            }}{{ i.introduction }}</p>
        </div>
...
const state = reactive({
  ...
  async getList(type) {
    ...
    let res = await get('/dataManage/home', {
      page: this.list.page,
      size: this.list.size
    });
    if (res.status) {
      ...
      state.showTotal = window.document.body.clientHeight / 200;
    }
  },
  animateScroll() {
...
onMounted(() => {
  state.getList(null);
  document.getElementById('webHome').addEventListener('scroll', (e) => {
    state.animateScroll();
  });
});
