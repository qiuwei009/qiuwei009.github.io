  ...
  <div class="w-100-p">
    ...
    <div class="w-100-p flex justify-center flex-reverse pt-10 pb-10">
          ...
          <div v-animation="'card-enter'" class="card opacity-0 m-10 p-10 radius-8">
            ...
            <ul class="indent-10">
              <li class="cursor" v-for="(row,index) in state.technology.records" :key="`a-${index}`"
                  @click="state.jump(`#message-${index}`)">{{ row.id }}、{{ row.title }}
              </li>
            </ul>
          </div>
        </Affix>
      ...
      <div class="main col-16 overflow-h">
        <main class="">
          <div v-for="(row,index) in state.technology.records" :id="`message-${index}`"
               :key="`c-${index}`" v-animation="'card-enter123 animate__animated animate__backInUp'" class="card opacity-0 m-10 p-10 radius-8 line-25">
